...
func (tf *txnFuture) wait() (kv.Transaction, error) {
	...
	if err == nil {
		if tf.pipelined {
			return tf.store.Begin(tikv.WithTxnScope(tf.txnScope), tikv.WithStartTS(startTS), tikv.WithPipelinedTxn())
		}
	...
	if tf.pipelined {
		return tf.store.Begin(tikv.WithTxnScope(tf.txnScope), tikv.WithPipelinedTxn())
	}
